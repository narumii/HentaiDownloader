...
import xyz.ethyr.downloader.object.GelBooruSite;
...
public class GelBooruDownloader extends Downloader {
    ...
    private static final String URL  = "https://gelbooru.com/index.php?page=dapi&s=post&q=index&pid=" + "%s" + "&limit=" + "%s" + "&tags=" + "%s" + "&api_key=16d7195f94cd43f7680c2310ec6788f05a6a6e06fbaad1f0e6c55fd284c57f5a&user_id=629393";
    ...
    public GelBooruDownloader(final File dir, final Scanner scanner) {
        ...
        imagePages.forEach((page, images) ->  new GelBooruSite(String.format(URL, page, images, StringUtil.join(
        ...
        ExecutorUtil.submit(()-> urls.forEach(site -> {
            try {
                ...
                for (int i = 0; i < site.getAmount(); i++) {
                    ...
                    Files
                        .copy(connection.getInputStream(), Paths.get(file.getPath(), image.getName() + "." + extension));
                }
